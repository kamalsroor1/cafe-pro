...
class TerminalTest extends TestCase
{
    ...
    /** @test */
    public function it_can_open_a_table_and_transition_to_order_mode()
    {
    ...
    /** @test */
    public function it_can_add_products_to_cart_and_persists_as_draft()
    {
        $table = RestaurantTable::factory()->create(['name' => 'T1']);
    ...
    /** @test */
    public function it_updates_existing_draft_when_adding_more_items()
    {
    ...
    /** @test */
    public function it_can_remove_items_from_cart_and_updates_database()
    {
        $table = RestaurantTable::factory()->create(['name' => 'T1']);
        $product = Product::factory()->create(['price' => 10.00]);
    ...
    /** @test */
    public function it_prevents_checkout_with_empty_cart()
    {
        Livewire::test(Terminal::class)
            ->call('checkout')
    ...
    /** @test */
    public function it_prevents_checkout_without_active_shift()
    {
        // Close the shift
    ...
    /** @test */
    public function it_completes_checkout_successfully()
    {
        $table = RestaurantTable::factory()->create(['name' => 'T1', 'status' => 'available']);
        ...
        // Check the draft order was deleted (since service creates a new paid one)
        // Note: Terminal.php:242 deletes the pending order
        ...
        // Check new order exists via service (assuming OrderService creates a paid status order)
        $this->assertDatabaseHas('orders', [
            'payment_status' => 'paid',
            'total' => 20.00,
        ]);
    ...
    /** @test */
    public function it_listens_for_shift_updated_event()
    {
        $component = Livewire::test(Terminal::class);

        // Manually change shift in DB
        $newShift = Shift::factory()->create(['status' => 'open']);

        // Dispatch event
        $component->dispatch('shiftUpdated');

        // Verify component picks it up
    }
}
